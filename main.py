...
from PySide6.QtCore import QTimer, Qt, QPointF, QRectF, QUrl, qInstallMessageHandler
...
for _d in (EXPORT_DIR, REPORT_DIR, CSV_DIR, REFERENCE_DIR, PROFILE_ASSET_DIR):
...
class OverlayWindow(QWidget):
    ...
    def paintEvent(self, event):
        ...
        painter.setBrush(QBrush(QColor(4, 8, 12, 115)))
        painter.drawRoundedRect(self.rect().adjusted(0, 0, -1, -1), 9, 9)
        painter.setBrush(QBrush(QColor(24, 34, 42, 75)))
...
class TireMiniWidget(QWidget):
    ...
    def paintEvent(self, event):
        ...
        for i in range(3):
            ...
            painter.setBrush(QBrush(QColor(5, 10, 15, 95)))
...
class TireOverlayWindow(QWidget):
    ...
    def paintEvent(self, event):
        ...
        painter.setRenderHint(QPainter.Antialiasing)
        painter.setPen(Qt.NoPen)
        painter.setBrush(QBrush(QColor(4, 8, 12, 120)))
        painter.drawRoundedRect(self.rect().adjusted(0, 0, -1, -1), 10, 10)
        painter.setBrush(QBrush(QColor(18, 26, 34, 78)))
...
class PedalOverlayWindow(QWidget):
    ...
    def paintEvent(self, _):
        ...
        # Karte
        p.setPen(Qt.NoPen)
        p.setBrush(QColor(13, 17, 23, 205))
...
class GeminiWindow(QWidget):
    ...
    def __init__(self, main):
        ...
        self.resize(760, 640)
        self.setStyleSheet(
            "QWidget { background:#0d1117; color:#e6ebf2; } "
            "QLabel { background:transparent; } "
            "QTextEdit, QLineEdit { background:#0f151e; color:#e6ebf2; border:1px solid #232c3a; border-radius:7px; padding:4px; } "
            "QLineEdit:focus, QTextEdit:focus { border:1px solid #ff5a1f; } "
            "QPushButton { background:#1b2431; color:#e6ebf2; border:1px solid #2c3849; padding:8px 13px; border-radius:7px; font-weight:600; } "
            "QPushButton:hover { background:#232f40; } "
            "QPushButton:pressed { background:#ff5a1f; border-color:#ff5a1f; color:#ffffff; }")
...
class Main(QMainWindow):
    def __init__(self):
        ...
        self.global_hotkey_state = {"F6": False, "F7": False, "F8": False, "F9": False, "F10": False}
        ...
        root.setStyleSheet("""
            QWidget { font-size: 13px; color: #e6ebf2; }
            QLabel { color: #e6ebf2; background: transparent; }
            QMainWindow, QWidget { background: #0d1117; color: #e6ebf2; }
            QTabWidget::pane { border: 1px solid #232c3a; border-radius: 10px; top: -1px; background: #10161f; }
            QTabBar::tab { background: transparent; color: #8b97a8; padding: 9px 16px; margin-right: 4px; border: none; border-bottom: 2px solid transparent; font-weight: 600; }
            QTabBar::tab:hover { color: #d6deea; }
            QTabBar::tab:selected { color: #ffffff; border-bottom: 2px solid #ff5a1f; }
            QGroupBox { border: 1px solid #232c3a; border-radius: 10px; margin-top: 14px; padding: 10px; font-weight: 700; background: #10161f; }
            QGroupBox::title { subcontrol-origin: margin; left: 12px; padding: 2px 6px; color: #ff7a45; }
            QPushButton { background: #1b2431; color: #e6ebf2; border: 1px solid #2c3849; padding: 8px 13px; border-radius: 7px; font-weight: 600; }
            QPushButton:hover { background: #232f40; border-color: #3a495e; }
            QPushButton:pressed { background: #ff5a1f; border-color: #ff5a1f; color: #ffffff; }
            QPushButton:disabled { color: #5a6675; background: #141a24; border-color: #232c3a; }
            QTextEdit, QLineEdit, QTableWidget, QComboBox { background: #0f151e; color: #e6ebf2; border: 1px solid #232c3a; border-radius: 7px; selection-background-color: #ff5a1f; selection-color: #ffffff; }
            QLineEdit:focus, QComboBox:focus, QTextEdit:focus { border: 1px solid #ff5a1f; }
            QComboBox { padding-left: 34px; padding-right: 8px; min-height: 30px; }
            QComboBox::drop-down { subcontrol-origin: padding; subcontrol-position: left center; width: 28px; border-right: 1px solid #232c3a; border-left: none; background: #1b2431; border-top-left-radius: 6px; border-bottom-left-radius: 6px; }
            QComboBox::drop-down:hover { background: #ff5a1f; }
            QComboBox::down-arrow { image: url(assets/dropdown_arrow_left.svg); width: 14px; height: 14px; }
            QComboBox QAbstractItemView { background: #0f151e; color: #e6ebf2; selection-background-color: #ff5a1f; selection-color: #ffffff; outline: 0; border: 1px solid #232c3a; }
            QTableWidget { gridline-color: #1c2530; alternate-background-color: #121a24; }
            QHeaderView::section { background: #161f2b; color: #b7c2d0; padding: 6px; border: none; border-right: 1px solid #232c3a; border-bottom: 2px solid #ff5a1f; font-weight: 700; }
            QTableWidget::item:selected { background: #ff5a1f; color: #ffffff; }
            QScrollBar:vertical { background: transparent; width: 12px; margin: 2px; }
            QScrollBar::handle:vertical { background: #2c3849; border-radius: 6px; min-height: 28px; }
            QScrollBar::handle:vertical:hover { background: #3a495e; }
            QScrollBar::add-line:vertical, QScrollBar::sub-line:vertical { height: 0; }
            QScrollBar:horizontal { background: transparent; height: 12px; margin: 2px; }
            QScrollBar::handle:horizontal { background: #2c3849; border-radius: 6px; min-width: 28px; }
            QScrollBar::handle:horizontal:hover { background: #3a495e; }
        ...
        subtitle = QLabel("0.4.7.5 Beta – KI-Analyse + Kontingent + Pedal-Trace + Sprachausgabe + Update-Pruefung")
        subtitle.setStyleSheet("font-size:14px;color:#b9c4d3;")
        ...
        primary = [self.btn_snapshot, self.btn_record, self.btn_stop, self.btn_report, self.btn_tire_overlay]
        overflow = [self.btn_csv, self.btn_ref_export, self.btn_ref_import, self.btn_pb_reference, self.btn_auto_bestlap, self.btn_overlay, self.btn_pedal_overlay, self.btn_export, self.btn_clear]
        for b in primary:
            buttons.addWidget(b)
        for b in overflow:
            buttons.addWidget(b)
        ...
        self.btn_more.setStyleSheet(
            "QToolButton { background:#1b2431; color:#e6ebf2; border:1px solid #2c3849; "
            "padding:8px 13px; border-radius:7px; font-weight:600; } "
            "QToolButton:hover { background:#232f40; } "
        ...
        _more.addAction("Monitor-Overlay umschalten", self.btn_overlay.click)
        _more.addAction("Pedal-Overlay umschalten", self.btn_pedal_overlay.click)
        _more.addSeparator()
        ...
        for b in [self.btn_profile_new, self.btn_profile_load, self.btn_profile_save, self.btn_profile_delete]:
            driver_bar.addWidget(b)
        ...
        for i, key in enumerate(["Verbindung", "Recording", "Runden", "Fahrer", "Setup", "Referenz", "Ext. Referenz", "PB-Bereich", "Auto-Bestlap", "Reifen", "Vergleich", "Coach-Fokus"]):
            ...
            frame.setStyleSheet("QFrame { background:#151d2a; border:1px solid #2b3545; border-radius:10px; padding:10px; }")
            fl = QVBoxLayout(frame)
            lab = QLabel(key); lab.setStyleSheet("color:#9fb0c4;font-size:12px;")
            val = QLabel("—"); val.setStyleSheet("font-size:18px;font-weight:bold;color:#ffffff;"); val.setWordWrap(True)
        ...
        hint = QLabel("Dropdowns + eigene Eingabe. Mehrere Buttonboxen separat. Screenshots werden in exports/profile_assets kopiert und im Report als KI-Vorbereitung markiert.")
        hint.setWordWrap(True)
        hw_btn_row.addWidget(hint)
        ...
        hw_layout.addWidget(QLabel("Hardware-Coach-Kontext"))
        hw_layout.addWidget(self.hardware_coach_text)
        ...
        for group_name, views in nav_groups:
            multi = len(views) > 1
            if multi:
                header = QStandardItem(group_name.upper())
                header.setFlags(Qt.NoItemFlags)
                header.setData(-1, Qt.UserRole)
                nav_model.appendRow(header)
            for v in views:
                if v not in tab_index:
                    continue
                item = QStandardItem(("    " + v) if multi else v)
        ...
        layout.addWidget(QLabel("System-Log / Debug"))
...
"""
    ...
    def setup_hotkeys(self):
        ...
        self.log.append("App-Hotkeys aktiv: F6 Reifen-Overlay · F7 Auto-Bestlap AN/AUS · F8 Recording Start/Stop · F9 Snapshot · F10 Coach-Overlay")
    ...
    def poll_windows_hotkeys(self):
        # VK codes: F6=0x75, F7=0x76, F8=0x77, F9=0x78, F10=0x79
        mapping = {"F6": 0x75, "F7": 0x76, "F8": 0x77, "F9": 0x78, "F10": 0x79}
    ...
    def update_live_lap_buffer(self, s: Sample):
        ...
        if len(self.live_lap_samples) > 900:
            self.live_lap_samples = self.live_lap_samples[-900:]
