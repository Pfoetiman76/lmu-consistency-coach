...
class TrackMapWidget(QWidget):
    ...
    def set_heat_metric(self, metric):
        self.heat_metric = metric if metric in ("pedal", "line") else "delta"
        self.update()
...
class Main(QMainWindow):
    def __init__(self):
        ...
        self.heatmap_metric_combo.addItems(["Färbung: Delta (Zeitverlust)", "Färbung: Pedal-Input (Gas/Bremse)",
                                           "Färbung: Linienabweichung (Meter)"])
        self.heatmap_metric_combo.currentIndexChanged.connect(
            lambda i: self.heatmap.set_heat_metric({1: "pedal", 2: "line"}.get(i, "delta")))
