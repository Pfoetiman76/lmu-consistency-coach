...
class WeatherOverlayWindow(QWidget):
    ...
    def paintEvent(self, _):
        ...
        p.setPen(Qt.NoPen); p.setBrush(QColor(13, 17, 23, 214))
        ...
        if s is None:
            p.setPen(QColor("#8a97a8")); p.drawText(QRectF(0, h / 2 - 10, w, 20), Qt.AlignCenter, "warte auf Telemetrie")
            p.end(); return
        ...
        def row(y, label, value, color):
            ...
            p.setPen(QColor("#8a97a8")); p.drawText(QRectF(10, y, 92, 18), Qt.AlignVCenter | Qt.AlignLeft, label)
        ...
        row(50, "Luft", f"{air:.1f} \u00b0C", QColor("#e6ebf2"))
        r_col = QColor("#58a6ff") if rain > 0.02 else QColor("#8a97a8")
...
class BrakeTempOverlayWindow(QWidget):
    ...
    def paintEvent(self, _):
        ...
        p.drawRoundedRect(QRectF(0, 0, w, h), 12, 12)
        f = QFont(); f.setPointSize(9); f.setBold(True); p.setFont(f)
        p.setPen(QColor("#8a97a8")); p.drawText(QRectF(10, 6, w - 20, 16), Qt.AlignLeft, "BREMSEN \u00b0C")
        if not self._live:
            p.setPen(QColor("#8a97a8"))
        ...
        for i in range(4):
            ...
            col = self._col(cval)
            p.setPen(QPen(QColor("#2b3545"), 1)); p.setBrush(QColor(21, 29, 42, 220))
            p.drawRoundedRect(QRectF(x, y, cw, ch), 7, 7)
            fl = QFont(); fl.setPointSize(8); p.setFont(fl); p.setPen(QColor("#8a97a8"))
        ...
        fn = QFont(); fn.setPointSize(7); p.setFont(fn); p.setPen(QColor("#8a97a8"))
        unit = "Kelvin erkannt \u2192 in \u00b0C" if (self._min_seen or 0) > 200.0 else "Rohwert als \u00b0C"
        p.drawText(QRectF(0, h - 14, w, 12), Qt.AlignCenter, unit)
        p.end()


class FuelOverlayWindow(QWidget):
    ...
    def paintEvent(self, _):
        ...
        p.drawRoundedRect(QRectF(0, 0, w, h), 12, 12)
        f = QFont(); f.setPointSize(9); f.setBold(True); p.setFont(f)
        p.setPen(QColor("#8a97a8")); p.drawText(QRectF(10, 6, w - 20, 16), Qt.AlignLeft, "KRAFTSTOFF")
        if not self._live:
            p.setPen(QColor("#8a97a8")); p.drawText(QRectF(0, h / 2 - 10, w, 20), Qt.AlignCenter, "warte auf Telemetrie")
            p.end(); return
        ...
        fs = QFont(); fs.setPointSize(10); p.setFont(fs); p.setPen(QColor("#8a97a8"))
        ...
        p.setPen(Qt.NoPen); p.setBrush(QColor(31, 39, 52)); p.drawRoundedRect(QRectF(bx, by, bw, bh), 5, 5)
        p.setBrush(col); p.drawRoundedRect(QRectF(bx, by, max(bw * min(pct, 1.0), 3), bh), 5, 5)

        def row(y, label, value, color=QColor("#e6ebf2")):
            fl = QFont(); fl.setPointSize(9); p.setFont(fl); p.setPen(QColor("#8a97a8"))
        ...
        row(112, "Reichweite", f"{ll:.1f} Runden" if ll else "\u2014",
            QColor("#f85149") if (ll is not None and ll < 2) else QColor("#e6ebf2"))
        if self.venergy > 0.0001:
            row(132, "Virt. Energie", f"{self.venergy * 100:.0f} %", QColor("#58a6ff"))
        p.end()


class FFBOverlayWindow(QWidget):
    ...
    def paintEvent(self, _):
        ...
        p.drawRoundedRect(QRectF(0, 0, w, h), 12, 12)
        f = QFont(); f.setPointSize(9); f.setBold(True); p.setFont(f)
        p.setPen(QColor("#8a97a8")); p.drawText(QRectF(10, 6, w - 20, 16), Qt.AlignLeft, "FORCE FEEDBACK")
...
class LimitOverlayWindow(QWidget):
    ...
    def _state(self):
        ...
        if not self._accel_seen and self._samples > 30:
            return (QColor("#8a97a8"), "kein Accel-Kanal", "--")
    ...
    def paintEvent(self, _):
        ...
        # Hintergrund
        p.setPen(Qt.NoPen)
        p.setBrush(QColor(13, 17, 23, 214))
        p.drawRoundedRect(QRectF(0, 0, w, h), 12, 12)
...
"""
    ...
    def refresh_overlay(self):
        ...
        for _a in ("weather_overlay_window", "brake_overlay_window",
                   "fuel_overlay_window", "ffb_overlay_window"):  # 0.4.9.0
