...
class BrakeCoach(QObject):
    """0.4.8.5 Brems-Countdown: extrahiert aus DEINER Live-Bestlap die harten
    Bremspunkte (Bremsbeginn-Distanz je Zone) und zaehlt vor jeder Zone zeitbasiert
    herunter - drei / zwei / eins / bremsen - per vorsynthetisierten Piper-Clips
    (latenzarm ueber winsound). Rein telemetriebasiert, kein Netz. Opt-in / F12.

    Wichtig: die Bremspunkte stammen aus deiner eigenen schnellsten Live-Runde
    (live_best_reference_lap). Faellt diese Referenz weg, schweigt der Countdown.
    """
    ...
    START_LEAD_S = 3.4       # ab hier wird ein Bremspunkt aktiv verfolgt
    BRAKE_LEAD_S = 0.20      # "bremsen" kurz vor dem Punkt
    ...
    def __init__(self, main):
        ...
        self.main = main
        self.enabled = False
        self.points = []          # [{"dist": m, "entry": kmh, "drop": kmh}]
        self.ref_lap = None
        self._active = None       # dist des aktuell verfolgten Bremspunkts
        self._said = set()        # bereits angesagte Marker fuer den aktiven Punkt
    ...
    # ---- Bremspunkte aus der Bestlap extrahieren -------------------------
    def set_reference_from_rows(self, rows):
        pts = self._extract_brake_points(rows)
        if pts:
            self.points = pts
            self.ref_lap = getattr(self.main, "live_best_reference_lap", None)

    def _extract_brake_points(self, rows):
        data = []
        for r in rows:
            try:
        ...
        for i in range(1, n):
            ...
            if armed and pb < self.BRAKE_ON <= b:
                # Eintrittstempo = hoechstes Tempo kurz vor dem Bremsbeginn
                entry = v
                k = i - 1
                while k >= 0 and (d - data[k][0]) < 25.0:
                    entry = max(entry, data[k][2]); k -= 1
                # Mindesttempo bis die Bremse wieder aus ist (Zonentiefe)
                minv = v; m = i
                while m < n and data[m][1] > self.BRAKE_OFF and (data[m][0] - d) < 400.0:
                    minv = min(minv, data[m][2]); m += 1
                if (entry - minv) >= self.HARD_DROP_KMH:
                    pts.append({"dist": d, "entry": entry, "drop": entry - minv})
                armed = False
            elif b < self.BRAKE_OFF:
                armed = True
        # nahe Punkte zusammenfassen, staerksten (groesster drop) behalten
        pts.sort(key=lambda p: p["dist"])
        merged = []
        for p in pts:
            if merged and (p["dist"] - merged[-1]["dist"]) < self.MERGE_DIST_M:
                if p["drop"] > merged[-1]["drop"]:
                    merged[-1] = p
            else:
                merged.append(p)
        return merged

    def _next_point(self, cur_dist):
        best = None
        for p in self.points:
            if p["dist"] > cur_dist and (best is None or p["dist"] < best["dist"]):
                best = p
        return best

    # ---- Countdown im live_tick ------------------------------------------
    def update(self, s):
        if not self.enabled or not self.points or s is None:
            return
        if getattr(self.main, "live_best_reference_lap", None) is None:
            return  # Referenz weg -> still
        try:
            if bool(getattr(s, "in_pits", False)) or bool(getattr(s, "lap_invalidated", False)):
        ...
        if cur < 0 or spd < self.MIN_SPEED_MPS:
            self._active = None; self._said = set(); return
        bp = self._next_point(cur)
        if bp is None:
            self._active = None; self._said = set(); return
        tt = (bp["dist"] - cur) / spd
        if self._active != bp["dist"]:
            if tt <= self.START_LEAD_S:
                self._active = bp["dist"]; self._said = set()
            else:
                return
...
class Main(QMainWindow):
    def __init__(self):
        ...
        self.act_brake_coach = _more.addAction("Brems-Countdown (3-2-1)")
...
"""
    ...
    def setup_hotkeys(self):
        ...
        self.log.append("App-Hotkeys aktiv: F6 Reifen-Overlay · F7 Auto-Bestlap AN/AUS · F8 Recording Start/Stop · F9 Snapshot · F10 Coach-Overlay · F11 Live-Coach · F12 Brems-Countdown")
    ...
    def set_brake_coach_enabled(self, on):
        ...
        if self.brake_coach_enabled:
            if not self.tts.is_available():
                self.log.append("Brems-Countdown an, aber keine Sprachausgabe – Piper/SAPI prüfen.")
            elif not self.brake_coach.has_points():
                self.log.append("Brems-Countdown an – noch keine Bremspunkte. Fahre eine saubere Bestlap.")
            self.status.setText("Brems-Countdown AN – Ansage vor harten Bremszonen (F12).")
        else:
            self.status.setText("Brems-Countdown AUS.")
